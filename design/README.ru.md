...
Здесь лежит то, из чего собран канвас редизайна. Задачи `D*`, которые на него
ссылались, выполнены целиком (см. раздел «Состояние» в [PLAN.md](../PLAN.md)); артборды
остаются визуальным референсом для текущего `public/app.css`.
...
| Файл | Что это |
| --- | --- |
| `refine.css` | **Предложение.** Слой добавлений к `public/app.css`: токены, компоненты, правки. Задача `D1` перенесла его токены в `public/app.css`; здесь осталось предложение в том виде, в каком оно нарисовано. |
