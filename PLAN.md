...
а не данных. Порядок метрик во всех списках (`display_order`) не вводится числом: строки
на `/metrics` перетаскиваются или двигаются стрелками, и после каждого шага позиции
перенумеровываются 1..n. Метрика без ссылок удаляется по-настоящему (это опечатка при создании),
метрика со ссылками уходит в архив: за ней стоит история.

## Ссылка на встречу

Ссылка на созвон живёт **на человеке**, а не на встрече: у 1:1 обычно постоянная комната,
и вводить её каждый раз было бы мучением. Кнопка «Открыть встречу» появляется в карточке
человека, в списке людей, в строке дашборда и в шапке самой встречи — то есть в тех местах,
откуда созвон реально начинают. Разовая ссылка на конкретную встречу — это отдельная
колонка на `meeting`; пока она никому не нужна.

Одно решение здесь про безопасность, а не про удобство. Пользовательский URL попадает в
`href`, а экранирование атрибута от `javascript:alert(1)` не спасает: строка проходит
экранирование невредимой и срабатывает по клику. Поэтому `lib/url.ts` пропускает только
`http` и `https`, а всё остальное **отклоняет, а не чистит** — ссылку, за которую нельзя
поручиться, мы не рендерим. Схема без префикса достраивается до `https://`, потому что
люди вставляют `meet.google.com/abc-defg-hij`, и отказывать в этом было бы придирками.

Проверка стоит на входе, а не на рендеринге: сохранить то, что мы отказались бы показать,
значит оставить поле, которое молча ничего не делает. Компонент `joinLink` всё равно
проверяет ссылку повторно — строки могут прийти из импорта или из правки базы руками,
а `href` неподходящее место, чтобы это выяснять. У ссылки `rel="noopener noreferrer"`,
чтобы открытая вкладка не имела доступа к приложению.

В саммари для подопечного ссылка не попадает: это внутреннее удобство, а приглашение на
встречу у человека и так есть. Тест это фиксирует.
...
Единственное исключение, зафиксированное в тесте, — каталог `src/i18n/` целиком; списка
файлов-исключений нет. Сид-миграции исключением не являются: содержимое шаблона это
пользовательский контент, который по проекту одноязычный, как и любой шаблон, написанный
руководителем, но стартовый шаблон видит каждый, кто поставил инструмент, поэтому он
написан по-английски, а руководитель, работающий на другом языке, переименует секции и
вопросы в конструкторе.
...
`direction = -1`. Стартовый шаблон «Regular 1:1» — подписи в сиде английские — с секциями:
как дела (shared), пульс (shared), развитие (shared), итог (shared), **приватная оценка
