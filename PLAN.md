...
Проверено: 37 тестов, `tsc --noEmit` чистый, сквозной сценарий пройден вживую
(человек → встреча → заполнение → завершение → график → снапшот → Markdown),
приватный сентинел не найден ни в одной поверхности шаринга.

Осталось до полного скоупа v1: редактор шаблонов (этап 4) и управление командами
в интерфейсе — сейчас команды создаются только скриптом демо-данных, хотя схема,
фильтр по команде и агрегаты готовы.
